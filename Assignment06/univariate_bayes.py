...
class UnivariateBayesianInference(object):
    ...
    def sample_rwm(self, n, w, param0):
        param=np.ones(n)
        param[0]=param0
        n_ac=0
        for i in range(n-1):
            x=param[i]
            y=stats.uniform.rvs(loc=x-w/2, scale=w, size=1, random_state=None) ##uniform distribution on [loc, loc + scale]
            alpha=min(1,self.quasi(y)/self.quasi(x))
            if rand()<alpha: 
                param[i+1]=y
                n_ac+=1
            else: param[i+1]=x
        return param,self.quasi(param),n_ac/n
