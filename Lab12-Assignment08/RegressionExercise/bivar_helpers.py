...
def betas_plot(fit, beta_0, beta_1, vector=False):
    """
    Make a scatterplot of the beta (coefficient) samples from
    StanFitResults instance `fit`.

    Draw a crosshair showing true values `beta_0` and `beta_1`.
    
    This supports `fit` objects using scalar params `fit.beta_0` and 
    `fit.beta_1`, or vector params in `fit.beta`, depending on the
    `vector` argument.
    """
    ...
    for c in range(nc):
        #using effective sample sizes: using space between samples to make samples almost independent
